...
export default function HomePage() {
  ...
  const pack = pets ?? [];
  const availableCount = countAvailable(pack);
  const featured = pickFeaturedPet(pack);
  ...
  if (error) {
    return (
      <RetryPanel
        message={error}
        onRetry={() => void reload()}
        className="max-w-7xl mx-auto px-8 py-48 flex flex-col items-center gap-6 text-center"
      />
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-12 md:py-24">
        ...
        <div className="md:col-span-5 flex flex-col gap-6 h-full justify-center relative z-10">
            ...
            <div className="bg-white border-4 border-slate-border rounded-geo-lg shadow-hard p-6 flex flex-col relative group z-10">
              ...
              <div className="w-full aspect-[4/5] bg-slate-100 rounded-geo border-2 border-slate-border overflow-hidden mb-4 relative">
                ...
                <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-sm border-2 border-slate-border p-4 rounded-xl">
                  <h3 className="font-black text-xl uppercase">{featured.name}</h3>
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
                    {[featured.breed, featured.age, featured.quote].filter(Boolean).join(' • ') || 'Ready to meet you'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="h-40 bg-secondary border-4 border-slate-border rounded-2xl p-6 flex items-center justify-between shadow-hard relative z-0">
            <div className="flex flex-col">
              <span className="text-5xl font-black italic">{loading ? '—' : pack.length}</span>
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-700">Pups in the Pack</span>
            </div>
            <div className="w-px h-full bg-slate-border opacity-20" />
      ...
      <section id="why-doodlepaws" className="mb-24 scroll-mt-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {[
            ...
            <div
              key={i}
              className="group bg-white p-8 border-4 border-slate-border rounded-geo shadow-hard hover:translate-y-1 hover:shadow-none transition-all"
            >
