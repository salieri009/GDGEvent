...
| `shared/api/client.ts` (frontend) | `ApiError`, fetch wrapper |

Frontend logical components: `app/App` → `features/*/pages/*Page` → `shared/api/client.ts` → BFF.
...
| Anon INSERT after v2 migration | Revoked by `v2-migration.sql` | **Mitigated** when operator runs migration |
...
- RLS anon INSERT on applications is **legacy/schema permissive** — target hardening in v2 ([ERD § v2 migration](erd.md): revoke anon INSERT + RPC-only writes).

### v2 RLS hardening (when migration applied)

| Control | Effect |
|---------|--------|
| Drop anon INSERT policy | Applications writable only via BFF service role |
| `submit_adoption_application` RPC | `SECURITY DEFINER`; granted to `service_role` only |
| Residual risk | Leaked service role key — rotate key; never in SPA bundle |
